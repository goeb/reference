...
gboolean accept_incoming_connection(gint fd, GIOCondition condition, gpointer user_data)
{
	...
	int *client_id = g_new0(int, 1);
	*client_id = next_client_id;
	next_client_id++;

	info("new client connected: %d", *client_id);

	g_unix_fd_add(client_fd, G_IO_IN|G_IO_PRI|G_IO_ERR|G_IO_HUP, receive_data, (gpointer)client_id);
